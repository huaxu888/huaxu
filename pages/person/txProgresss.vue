<template>
	<view style="min-height:100vh;background:white;">
		...
		<view>
			...
			<!-- <view class="padding flex justify-between align-center" style="background:#F8F8F8">
				<view class="text-lg">
					{{pikerData.value}}
				</view>
				...
				>
					  <view class="flex align-center text-lg text-black text-bold">
						...
						<view class="bbggii" style="width: 50upx;height: 47upx;background-image: url('https://img.91goodluck.com/jsmx_03.png');">
			...
			<view>
				<view class="flex padding justify-between align-center" v-for="(item,i) of infoList" :key="i" style="border-bottom: 0.1px solid #e4e4e4;"> 
					<view class="flex align-center">						
						<view v-if="item.BankNo.length!=0" class="bbggii hxIcon-xiaofeitixian" style="font-size: 48upx;color: #ff5b2e;">
						...
						<view class="margin-left-sm">
							<view>
								提现到-{{item.BankName}}({{after4(item.BankNo)}})
							</view>
	...
	export default{
		...
		methods:{
			...
			getTime(){
		 	...
		 	async getCurryInfo(){
				let info = await this.$http.tXzd(this.$store.state.userInfo.ID)
				console.log(info);
