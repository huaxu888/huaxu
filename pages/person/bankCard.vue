	...
	export default {
		data() {
			return {
				...
				BankNos:this.$store.state.userInfo.BankNo,
				StoreID:0
			}
		},
		onLoad(router) {
			this.list.title = this.$store.state.userInfo.BankName
			this.list.name = this.$store.state.userInfo.BankUserName
			this.list.BankNo = this.$store.state.userInfo.BankNo
		...
		methods: {
			...
			click(index, index1) {
				if(index1 == 1) {
					...
					this.$store.state.userInfo.BankName = '';
					this.$store.state.userInfo.BankUserName = '';
					this.$store.state.userInfo.BankNo = '';
				}
				console.log(this.$store.state.userInfo.ID);
				let userid = this.$store.state.userInfo.ID
				this.$http.shanKa(userid).then(res=>{
