	...
	<view style="background-color: #F2F2F2;">
		...
		<view style="background-color: #ffffff;padding-top: 30upx;">
			...
			<view class="cu-form-group" style="border-bottom: 1upx solid #eeeeee;">
				...
				<button class='cu-btn shadow text-white' style="background-color: #fe5479;font-size: 24upx;padding: 10upx;" @tap="getCode">{{ codeBth }}</button>
			</view>
		</view>
	...
	export default {
		...
		onLoad(router) {
			...
			this.$Request.get(this.$store.state.getShopBankList,{StoreID: this.StoreID}).then(res=>{
				console.log(res);
				if(res.IsSuccess){
					this.name = res.Data[0].BankUserName;
					this.bank = res.Data[0].BankName;
					this.cardNum = res.Data[0].BankNo;
					this.font  = '修改'
				}
			})
		},
											...
											uni.request({
												...
												success: function(res) {
													if (res.data.IsSuccess) {
														uni.request({
															url: 'https://newsapp.huaxuapp.com/api/storebank/addbank',
															data: {
...
<style scoped lang="scss">
	...
	.sure {
		...
		// box-shadow: 2upx 2upx 14upx lighten($color: #fe5479, $amount: 10);
	}
</style>
