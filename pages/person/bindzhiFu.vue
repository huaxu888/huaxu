<template>
	<view>
		...
		<cu-custom bgColor="bg-whitesss" :isBack="true" class="text-black">
			<!-- #ifdef APP-PLUS || H5-->
			<block slot="content">支付宝管理</block>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<block slot="content">支付宝管理</block>
			<!-- #endif -->
			...
			<view>
				<view class="text-center flex justify-between" style="background-color: #FFFFFF;margin-top: 30upx;padding:30upx;" @tap="bindBank()">
					<view>
						...
						<text style="font-size: 28upx;">添加支付宝</text>
					</view>
					<view>
	...
	export default {
		data() {
			return {
				...
				userInfo:'',
				BankNos:this.$store.state.userInfo.ZhiFuBaoNo
			}
		},
		onLoad(router) {
		...
		methods: {
			...
			click(index, index1) {
				if(index1 == 1) {
					...
					this.$u.toast(`删除成功`);
					this.$store.state.userInfo.ZhiFuBaoNo = '';
					this.$store.state.userInfo.ZhiFuBaoNick = '';
					this.$store.state.userInfo.ZhiFuBaoID = '';
				}
				console.log(this.$store.state.userInfo.ID);
				let userid = this.$store.state.userInfo.ID
				this.$http.jbZfb(userid,this.$store.state.userInfo.ZhiFuBaoNo).then(res=>{
			...
			open(index) {
				...
				this.lists.map((val, idx) => {
					if(index != idx) this.lists[idx].show = false;
				})
			}
		},
		components: {}
