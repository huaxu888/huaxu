<template>
	<view>
		...
		<view class="margin padding-bottom padding-top-xs" style="position: relative">
			...
			<view class=" margin-top-sm flex padding-sm margin-bottom align-center justify-between" style="position: relative; border-bottom: 1px solid #F0F0F0;background: #FFFFFF;border-radius: 10upx;">
				<view class="flex align-center justify-center">
					<text class=" " :class="TransferSort==2?'hxIcon-yuesel hx-text-red':'hxIcon-yue1 text-yellow'" style="font-size: 50upx;"></text>
					<text style="font-size: 32upx;margin-left: 24upx;line-height: 1em;"> {{TransferSort==2?'可消费余额':'余额'}}</text>
				</view>
				<view class="flex align-center justify-center">
					<text>&yen;{{}}</text>
				</view>
			...
			<view class="text-bold   margin-bottom">收款账户</view>
			...
			<view class=" margin-top  flex align-center padding" style=" border-bottom: 1px solid #F0F0F0; background: #FFFFFF;border-radius: 10upx">
				...
				<input type="digit" placeholder-style="font-size: 17px;color: #ddd;" v-model="money" maxlength="11" :adjust-position="false"
				 confirm-type="done" :placeholder="`可转账余额：${KeTiXian}元`" @input="changeMoney(money)" style="font-size: 65upx;height: 80upx; " />
				<text style="font-size: 30upx;white-space:nowrap; color: #EC3B46;font-size: 28upx;" class="margin-right-xs" @tap="allWithDrawal()">全部</text>
			</view>
			<view class="text-gray margin-top-xs" style="margin-bottom: 90upx;">请确认对方的手机号码无误，避免损失</view>
...
<script>
	...
	export default {
		...
		methods: {
			allWithDrawal() {
				let currentMoney = this.$api.formatAmount(this.KeTiXian);
				console.log(currentMoney);
				this.money = Number(currentMoney);
			...
			async getBalance() {
				if (this.userInfo_.ID) {
					...
					this.Total = this.$api.formatAmount(data.Data.Total);
				} else {
					this.KeTiXian = 0
					this.Total = 0;
				}
