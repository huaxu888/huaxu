			...
			<view>
				<view class="flex padding justify-between align-center" v-for="(item,i) of infoList" :key="i" style="border-bottom: 0.1px solid #e4e4e4;"> 
					<view class="flex align-center">						
						<view class="bbggii" style="width: 46upx;height: 46upx;background-image: url('https://img.91goodluck.com/jsmx_07.png');">
	...
	export default{
		...
		methods:{
			...
			getTime(){
		 	...
		 	async getCurryInfo(){
				let info = await this.$http.zzZd(this.$store.state.userInfo.ID)
				console.log(info);
