<template>
	<view class="">
		...
		<cu-custom bgColor="bg-whitesss" class="text-black" :isBack="true" style="border-bottom: 1upx solid #e4e4e4;">
			<!-- #ifdef APP-PLUS || H5-->
			<block slot="content">自发券账户</block>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<block slot="content">自发券账户</block>
			<!-- #endif -->
		...
		<view style="background-color: #FFFFFF;text-align: center;padding-top: 30upx;border-radius:8upx;padding-bottom: 40upx;">
			...
			<view style="margin-top: 30upx;">
				<text style="font-size: 64upx;font-weight: 600;" v-if="zfJin"><text style="font-size: 32upx;font-weight: 0;">￥</text>{{changeMoney(zfJin)}}</text>
				<text style="font-size: 64upx;font-weight: 600;" v-else><text style="font-size: 32upx;font-weight: 0;">￥</text>{{changeMoney(zfJin)}}</text>
			</view>
		</view>
		...
		z-index: 9;-moz-box-shadow:0px -3px 9px 0px #e9e9e9;-webkit-box-shadow:0px -3px 9px 0px #e9e9e9;box-shadow:0px -3px 9px 0px #e9e9e9;" v-if="zfJin>=100">
...
<script>
	...
	export default {
		...
		onLoad(router) {
			...
			this.yue = this.getTimes()
			this.$http.txMx(this.StoreID,this.yue,2,1,200).then(res => {
				console.log(res);
		...
		methods: {
			...
			tiXian() {
				uni.navigateTo({
					url:'/pages/shopManagement/sonPage/witHdraw/zfwitHdraw?StoreID=' + this.StoreID + '&zfJin=' + this.zfJin
				})
			},
			doTouchstart(index) {
				switch (index) {
