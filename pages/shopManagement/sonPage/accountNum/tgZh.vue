	...
	<view class="">
		...
		<view style="background-color: #FFFFFF;text-align: center;padding-top: 30upx;border-radius:8upx;padding-bottom: 40upx;">
			...
			<view style="margin-top: 30upx;">
				<text style="font-size: 64upx;font-weight: 600;" v-if="tgJin"><text style="font-size: 32upx;font-weight: 0;">￥</text>{{changeMoney(tgJin)}}</text>
				<text style="font-size: 64upx;font-weight: 600;" v-else><text style="font-size: 32upx;font-weight: 0;">￥</text>{{changeMoney(tgJin)}}</text>
			</view>
		</view>
		...
		<view class="margin flex justify-between">
			...
			<view class="flex align-center" style="background-color: #ffffff;border-radius: 40upx;width: 180upx;height: 60upx;margin-top: 10upx;" @change="changeDayss">
				...
				<text class="cuIcon-triangledownfill" style="font-size: 36upx;margin-left: 6upx;"></text>
			</view>
		</view>
		
		<view style="background-color: #ffffff;padding-bottom: 0upx;padding-top:50upx;" class="padding" v-if="zdList.length > 0">
			<view class="flex justify-between padding" style="background-color: rgba(246,179,127,0.1);font-size: 26upx;border-radius: 10upx;">
		...
		<view style="background-color: #ffffff;padding-bottom: 260upx;" class="padding" v-if="zdList.length > 0">
			<view class="flex" style="border-bottom: 1upx solid #eeeeee;padding: 28upx 0;" v-for="(item,i) in zdList" :key="i">
				...
				<view v-if="item.State" style="">
					<text class="hxIcon-gou" style="font-size: 36upx;color: #ff5b2e;"></text>
				</view>
				<view style="margin-left: 20upx;">
					<view style="">
		...
		z-index: 9;-moz-box-shadow:0px -3px 9px 0px #e9e9e9;-webkit-box-shadow:0px -3px 9px 0px #e9e9e9;box-shadow:0px -3px 9px 0px #e9e9e9;" v-if="tgJin>=100">
			<view style="text-align: center;line-height: 80upx;width: 690upx;height: 80upx;background-color: rgb(254,84,121);
...
<script>
	...
	export default {
		...
		onLoad(router) {
			...
			this.yue = this.getTimes()
			this.$http.txMx(this.StoreID,this.yue,3,1,200).then(res => {
				console.log(res);
		...
		methods: {
			...
			tiXian() {
				uni.navigateTo({
					url:'/pages/shopManagement/sonPage/witHdraw/tgwitHdraw?StoreID=' + this.StoreID + '&tgJin=' + this.tgJin
				})
			},
			doTouchstart(index) {
				switch (index) {
