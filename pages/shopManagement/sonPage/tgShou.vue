	...
	<view>
		<view class="bill_details">
			...
			<cu-custom bgColor="bg-whitesss" class="text-white" :isBack="true">
				<!-- #ifdef APP-PLUS || H5-->
				<block slot="content">团购收款明细</block>
				<!-- #endif -->
				<!-- #ifdef MP-WEIXIN -->
				<block slot="backText">团购收款明细</block>
				<!-- #endif -->
			</cu-custom>
			<!-- #endif -->
		</view>
				...
				<view class="flex  align-center justify-between" style="background-color: #e9a555;padding: 30upx 30upx;border-radius: 18upx;margin-top: 30upx;">
					<view class="bg_title flex flex-direction align-center justify-center" style="width: 33%;">
						<view class="flex align-center" style="margin-bottom: 4upx;">
							<text class="text-gray text-df" style="color: #f8e1c8;font-size: 26upx;">今日团购总额</text>
						</view>
		...
		<view  v-if="curNow==1"  style="background-color: #FFFFFF;">
			...
			<view style="width:100%;padding: 0 30upx;margin-top: 70upx;">
				<view class="flex  align-center justify-between" style="background-color: #e9a555;padding: 30upx 30upx;border-radius: 18upx;margin-top: 30upx;">
					<view class="bg_title flex flex-direction align-center justify-center" style="width: 33%;">
						<view class="flex align-center" style="margin-bottom: 4upx;">
							<text class="text-gray text-df" style="color: #f8e1c8;font-size: 26upx;">本月团购总额</text>
						</view>
	...
	export default {
		...
		onLoad(route) {
			...
			if ('billType' in route) {
				...
				this.$http.cxRibao(this.getData.StoreID,0, this.getData.day,2,this.page,this.pagesize)
				.then(res => {
					console.log(res);
		...
		methods: {
			...
			changeDay(e) {
				...
				this.classIndex = 0
				this.$http.cxRibao(this.getData.StoreID,0,this.datas,2,this.page,this.pagesize)
				.then(res => {
					console.log(res);
					this.jryye = res.jryye
					this.TotalOrder = res.TotalOrder
			...
			changeDays(e) {
				...
				this.getData.page = 1
				this.getCurryInfo()
				this.$http.cxYuebao(this.getData.StoreID,this.timeValue,2,0,this.page,this.pagesize)
				.then(res => {
					console.log(res);
			...
			classPickerChange(e) {
				...
				if(this.curNow == 0) {
					this.$http.cxRibao(this.getData.StoreID,this.getData.yyyid,this.datas,2,this.page,this.pagesize)
					.then(res => {
						console.log(res);
						this.jryye = res.jryye
						this.TotalOrder = res.TotalOrder
				...
				if(this.curNow == 1) {
					this.$http.cxYuebao(this.getData.StoreID,this.timeValue,2,this.getData.yyyid,this.page,this.pagesize)
					.then(res => {
						console.log(res);
						this.jryyes = res.jryye
						this.TotalOrders = res.TotalOrder
