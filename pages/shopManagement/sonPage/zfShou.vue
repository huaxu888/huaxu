	...
	<view>
		<view class="bill_details">
			...
			<cu-custom bgColor="bg-whitesss" class="text-white" :isBack="true">
				<!-- #ifdef APP-PLUS || H5-->
				<block slot="content">自发券收款明细</block>
				<!-- #endif -->
				<!-- #ifdef MP-WEIXIN -->
				<block slot="content">自发券收款明细</block>
				<!-- #endif -->
				...
				<view class="flex  align-center justify-between" style="background-color: #e9a555;padding: 30upx 30upx;border-radius: 18upx;margin-top: 30upx;">
					<view class="bg_title flex flex-direction align-center justify-center" style="width: 33%;">
						<view class="flex align-center" style="margin-bottom: 4upx;">
							<text class="text-gray text-df" style="color: #f8e1c8;font-size: 26upx;">今日实收总额</text>
						</view>
						<text class="padding-top-sm" style="font-size:45upx;color:#ffffff"><text style="font-size: 32upx;">￥</text>{{changeMoney(jryye)}} </text>
					</view>
					<view class="bg_title flex flex-direction align-center justify-center" style="width: 33%;">
						<view class="flex align-center" style="margin-bottom: 4upx;">
							<text class="text-gray text-df" style="color: #f8e1c8;font-size: 26upx;">验券数</text>
						</view>
					...
					<u-time-line-item v-for="(item,i) of infoObjs" :key="i" >
						<template v-slot:content >
							...
							<view style="position: absolute;top: 60upx;right: 60upx;font-weight: 600;"   @tap="goToDetail(item)">
								<text  style="font-size: 34upx;">+{{item.SJXFJE}}</text>
							</view>
						</template>
					</u-time-line-item>
				</u-time-line>
		...
		<view  v-if="curNow==1"  style="background-color: #FFFFFF;">
			...
			<view style="width:100%;padding: 0 30upx;margin-top: 70upx;">
				<view class="flex  align-center justify-between" style="background-color: #e9a555;padding: 30upx 30upx;border-radius: 18upx;margin-top: 30upx;">
					<view class="bg_title flex flex-direction align-center justify-center" style="width: 33%;">
						<view class="flex align-center" style="margin-bottom: 4upx;">
							<text class="text-gray text-df" style="color: #f8e1c8;font-size: 26upx;">本月实收总额</text>
						</view>
	...
	export default {
		...
		onLoad(route) {
			...
			if ('billType' in route) {
				...
				this.$http.cxRibao(this.getData.StoreID,0, this.getData.day, 1,this.page,this.pagesize)
				.then(res => {
					console.log(res);
		...
		methods: {
			...
			changeDay(e) {
				...
				this.classIndex = 0
				this.$http.cxRibao(this.getData.StoreID,0,this.datas,1,this.page,this.pagesize)
				.then(res => {
					console.log(res);
					this.jryye = res.jryye
					this.TotalOrder = res.TotalOrder
			...
			changeDays(e) {
				...
				this.getData.page = 1
				this.getCurryInfo()
				this.$http.cxYuebao(this.getData.StoreID,this.timeValue,1,0,this.page,this.pagesize)
				.then(res => {
					console.log(res);
			...
			classPickerChange(e) {
				...
				if(this.curNow == 0) {
					this.$http.cxRibao(this.getData.StoreID,this.getData.yyyid,this.datas,1,this.page,this.pagesize)
					.then(res => {
						console.log(res);
						this.jryye = res.jryye
						this.TotalOrder = res.TotalOrder
				...
				if(this.curNow == 1) {
					this.$http.cxYuebao(this.getData.StoreID,this.timeValue,1,this.getData.yyyid,this.page,this.pagesize)
					.then(res => {
						console.log(res);
						this.jryyes = res.jryye
						this.TotalOrders = res.TotalOrder
			...
			goToDetail(itemObjs) {
				...
				uni.navigateTo({
					url: `/pages/shopManagement/sonPage/transactionDetailsPages?XFID=${itemObjs.XFID}&Sort=${itemObjs.Sort}&XFJE=${itemObjs.XFJE}&UserID=${itemObjs.UserID}&Nick=${itemObjs.Nick}&yyyPhone=${itemObjs.Phone}&info=${itemObjs.Info}&Name=${itemObjs.Name}&SJXFJE=${itemObjs.SJXFJE}&sqt=9`
				})
				return
			},
