<template>
	<view>
		...
		<cu-custom bgColor="bg-whitesss" :isBack="true" class="text-black">
			<!-- #ifdef APP-PLUS || H5-->
			<block slot="content">团购收款提现</block>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<block slot="content">团购收款提现</block>
			<!-- #endif -->
					...
					<view class="item u-border-bottom" >
						...
						<view class="title-wrap" style="margin-left: 20upx;">
							<view style="margin-top: 10upx;">
								...
								<text style="padding: 6upx 12upx;background-color: #f5e9e2;color: #f86b40;font-size: 24upx;margin-left: 10upx;border-radius: 5upx;">默认</text>
							</view>
							<view style="margin-top: 10upx;">
		...
		<view class="padding" style="margin: 30upx;padding-bottom: 60upx;background-color: #ffffff;border-radius: 20upx;">
			...
			<view class=" margin-top  flex align-center " style="border-bottom: 1px solid #F0F0F0; margin-bottom: 0upx;">
				...
				<input type="digit" placeholder-style="font-size: 14px;color: #ddd;"  maxlength="11"
				 :adjust-position="false" confirm-type="done" placeholder="" :value="changeMoney(tgJin)" style="font-size: 65upx;height: 80upx; " disabled/>
			...
			<view class="margin-tb  text-sm" style="color:#b9b9b9;">可提现金额
				<text style="color:#333333;">{{changeMoney(tgJin)}}</text>元
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			bindUser() {
				uni.redirectTo({
					url:'/pages/shopManagement/sonPage/bindUsers/bindUser?StoreID=' + this.StoreID + '&iid=3'
				});
			},
			times() {
				var date = new Date();
			...
			fullclose: async function(res) {
				...
				if (IsSuccess) {
					this.getData.num = this.zfJin
					this.getData.userid = this.$store.state.userInfo.ID
					this.getData.sort = this.sorts
					this.getData.StoreID = this.StoreID
					
					this.$http.tgTix(this.getData.StoreID,this.getData.num,this.getData.userid,this.getData.pwd,this.getData.sort).then(res=>{
						if(res.IsSuccess){
							setTimeout(()=>{
								this.$api.msg(res.Msg,2000,'success')
							},500)
							uni.redirectTo({
								url:`/pages/prestoreDetail/isSuccess?type=tx&money=${this.tgJin}&StoreID=${this.StoreID}&userid=${this.getData.userid}&sort=3`
							})
