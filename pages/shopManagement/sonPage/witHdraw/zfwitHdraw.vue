<template>
	<view>
		...
		<cu-custom bgColor="bg-whitesss" :isBack="true" class="text-black">
			<!-- #ifdef APP-PLUS || H5-->
			<block slot="content">自发券收款提现</block>
			<!-- #endif -->
			<!-- #ifdef MP-WEIXIN -->
			<block slot="content">自发券收款提现</block>
			<!-- #endif -->
		...
		<view class="padding" style="margin: 30upx;padding-bottom: 60upx;background-color: #ffffff;border-radius: 20upx;">
			...
			<view class=" margin-top  flex align-center " style="border-bottom: 1px solid #F0F0F0; margin-bottom: 0upx;">
				...
				<input type="digit" placeholder-style="font-size: 14px;color: #ddd;"  maxlength="11"
				 :adjust-position="false" confirm-type="done" placeholder="" :value="changeMoney(zfJin)" style="font-size: 65upx;height: 80upx; " disabled/>
			...
			<view class="margin-tb  text-sm" style="color:#b9b9b9;">可提现金额
				<text style="color:#333333;">{{changeMoney(zfJin)}}</text>元
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			bindUser() {
				uni.redirectTo({
					url:'/pages/shopManagement/sonPage/bindUsers/bindUser?StoreID=' + this.StoreID + '&iid=2'
				});
			},
			times() {
				var date = new Date();
			...
			fullclose: async function(res) {
				...
				if (IsSuccess) {
					...
					this.$http.zfTix(this.getData.StoreID,this.getData.num,this.getData.userid,this.getData.pwd,this.getData.sort).then(res=>{
						if(res.IsSuccess){
							setTimeout(()=>{
								this.$api.msg(res.Msg,2000,'success')
							},500)
							uni.redirectTo({
								url:`/pages/prestoreDetail/isSuccess?type=tx&money=${this.zfJin}&StoreID=${this.StoreID}&userid=${this.getData.userid}&sort=2`
							})
