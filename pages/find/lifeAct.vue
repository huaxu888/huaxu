...
<script>
	export default {
		...
		onShow() {
			...
			uni.getLocation({
				success: res => {
					...
					this.$http.myselfTimelist(4,0,1,10,this.location || '107.02,33.07').then(res => {
						console.log(res);
		...
		onReachBottom() {
			this.currentPage += 1
			this.$http.myselfTimelist(4,0,this.currentPage,10,this.location || '107.02,33.07').then(res => {
				console.log(res);
				if (res.length > 0) {
