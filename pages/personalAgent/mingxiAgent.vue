	...
	<view class="">
		...
		<view class="margin flex justify-between">
			<view style="margin-top: 20upx;">
				<text style="font-size: 28upx;">账户明细</text>
			</view>
		...
		<view style="background-color: #ffffff;padding-bottom: 0upx;padding-top:50upx;" class="padding" v-if="zdList.length > 0">
			<view class="flex justify-between padding" style="background-color: rgba(255,230,232,0.4);font-size: 26upx;border-radius: 10upx;">
				<text style="color: #333333;">共{{agentcount}}笔</text>
				<text style="color: #999999;">提现<text style="color: #333333;font-size: 26upx;">￥<text style="font-size: 28upx;">{{changeMoney(agentTotalSum)}}</text></text></text>
				<text style="color: #999999;">实到<text style="color: #333333;font-size: 26upx;">￥<text style="font-size: 28upx;">{{changeMoney(agenttotalin)}}</text></text></text>
			</view>
		</view>
		...
		<view style="background-color: #ffffff;padding-bottom: 260upx;" class="padding" v-if="zdList.length > 0">
			<view class="flex" style="border-bottom: 1upx solid #eeeeee;padding: 28upx 0;" v-for="(item,i) in zdList" :key="i">
				...
				<view v-if="item.State" style="">
					<text class="hxIcon-gou" style="font-size: 36upx;color: #fe4e01;"></text>
				</view>
				<view style="margin-left: 20upx;">
					<view style="">
				...
				<view style="text-align: right;position: absolute;right: 30upx;width: 100%;">
					...
					<view style="color: #fe4e01;margin-top: 10upx;" v-if="!item.State">
		...
		<view style="text-align: center;height: 400upx;margin-top: 180upx;" v-if="zdList.length == 0">
			<image src="https://img.huaxuapp.com/wukong.png" mode="aspectFill" style="width: 376upx;height: 250upx;"></image>
			<view style="margin-top: 20upx;">
				<text style="font-size: 28upx;color: #333333;">没有账户明细哦~</text>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		onLoad(router) {
			...
			this.yue = this.getTimes()
			this.$http.personTixianList(this.$store.state.userInfo.ID,this.yue,1,15).then(res => {
				console.log(res);
				this.zdList = res.Data
				
				this.agentcount = res.agentcount
				this.agentTotalSum = res.agentTotalSum
				this.agenttotalin = res.agenttotalin
			})
			.catch(err => {
				console.log(err);
			}) 
		...
		onReachBottom() {
			...
			this.$http.personTixianList(this.$store.state.userInfo.ID,this.yue,this.pages,15).then(res => {
