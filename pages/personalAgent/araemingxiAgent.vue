<template>
	<view style="background:white;">
		...
		<view v-if="infoList.length  > 0" class="flex padding justify-between align-center" v-for="(item,i) of infoList" :key="i" style="border-bottom: 0.1px solid #e4e4e4;"> 
			<view class="flex align-center">						
				
				<image :src="item.LogoPic || 'https://img.huaxuapp.com/6454608_.pic.jpg'" mode=""style="border-radius: 76upx;width: 76upx;height: 76upx;z-index=1"></image>
		...
		<view style="text-align: center;height: 400upx;margin-top: 180upx;" v-if="infoList.length == 0">
...
<script>
	export default{
		...
		onLoad(e){
			...
			this.getTime()
			this.$http.areaShopTeam(this.$store.state.userInfo.ID,this.yue,this.ckday,1,15).then(res => {
				console.log(res);
				if(res.IsSuccess){
		...
		onReachBottom(){
			...
			this.$http.areaShopTeam(this.$store.state.userInfo.ID,this.yue,this.ckday,this.pages,15).then(res => {
				console.log(res);
