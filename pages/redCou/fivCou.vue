	...
	<view style="height: 100%;position: relative;">
		<!-- #ifdef MP-WEIXIN || APP-PLUS || H5 || MP-ALIPAY -->
			<cu-custom  bgColor="bg-whitesss" :isBack="true" class="text-black bgclo"  style="height: 100%;">
				<block slot="content" class="text-bold bgclo" style="width: 100%;">五折好券</block>
			</cu-custom>
		<!-- #endif -->
...
<script>
	export default {
		...
		onShow() {
			uni.getLocation({
				success: res => {
					...
					this.$http.fivCou(0.5,this.currentPage,this.pagesize,this.location || '107.02,33.07').then(res => {
		...
		async onReachBottom() {
			...
			await this.$http.fivCou(0.5,this.currentPage,this.pagesize,this.location || '107.02,33.07').then(res => {
				if (res.Data.length > 0) {
