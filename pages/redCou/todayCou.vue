	...
	<view style="height: 100%;position: relative;">
		<!-- #ifdef MP-WEIXIN || APP-PLUS || H5 || MP-ALIPAY -->
			<cu-custom  bgColor="bg-whitesss" :isBack="true" class="text-black bgclo"  style="height: 100%;">
				<block slot="content" class="text-bold bgclo" style="width: 100%;">今日新券</block>
			</cu-custom>
		<!-- #endif -->
	...
	export default {
		...
		onShow() {
			uni.getLocation({
				success: res => {
					...
					this.currentPage = 1
					this.$http.todayCou(this.currentPage,this.pagesize,this.location || '107.02,33.07').then(res => {
		...
		methods: {
			...
			goGod(e){
				if(e == 0){
					uni.navigateTo({
						url:'/pages/index/redCou/todayCou'
					})
				}
				if(e == 1){
					uni.navigateTo({
						url:'/pages/index/redCou/fivCou'
					})
				}
				if(e == 2){
					uni.navigateTo({
						url:'/pages/index/redCou/bigCou'
					})
				}
				// if(e == 3){
				// 	uni.navigateTo({
		...
		async onReachBottom() {
			...
			await this.$http.todayCou(this.currentPage,this.pagesize,this.location || '107.02,33.07').then(res => {
				if (res.Data.length > 0) {
